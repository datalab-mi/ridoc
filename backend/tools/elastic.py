    ...
    if must:
        if expression_file:
            ...
            with open(expression_file, 'r') as f:
                content = f.read()
            list_expression = [x.lower().split(' ')
                               for x in str(content).split('\n')]
        ...
        analyzed = [ananyse_tokens['token']
                    for ananyse_tokens in analyse["tokens"]]
...
def search(must: dict, should: dict, filter: dict, index_name: str,
    ...
    if (must or should or filter):
        ...
        time.sleep(0.5)
        D = es.search(index=index_name, body=body, size=thresholds.get(
            'd_threshold', int(1000)), request_cache=False)
    else:
    ...
    try:  # This try is for the case where no match is found
        # The first filter then the second filter
        if not T and D['hits']['hits'][0]["_score"]/length_of_request < size:
            Bande = True
    except:
        pass
    # import pdb; pdb.set_trace()
...
def delete_alias(index_name: str, alias_name: str):
    es.indices.delete_alias(
        index=index_name, name=alias_name, ignore=[400, 404])
    ...
    with open(synonym_file, 'w') as outfile:
        ...
        if expression_file:
            ...
            with open(os.path.join(user_data, expression_file), 'r') as f2:
                ...
                list_expression = [x.lower().split(' ')
                                   for x in str(f2.read()).split('\n') if x != '']
                list_expression = [' '.join(
                    x) + ' => ' + '_' + ''.join(x) + '_, ' + ', '.join(x) for x in list_expression]
        ...
        if glossary_file:
            ...
            with open(os.path.join(user_data, glossary_file), 'r') as f1:
                list_glossary = [x.split(' => ') for x in str(f1.read()).split(
                    '\n') if '=>' in x]
                list_glossary = [x[0].replace(
                    ' ', ', ') + ' => ' + x[1] for x in list_glossary]
                str_glossary = '\n'.join(list_glossary)
...
def index_file(filename: str, index_name: str, user_data: str, dst_path: str,
    ...
    # clean date field
    if "date" in data:
        # remove all caractere ! /, - or digit
        data["date"] = re.sub(r'[^\d\/-]', '', data["date"])
    ...
    for entry in sections:
        if entry.get('tag', False):
            # import pdb; pdb.set_trace()
            res_tag = get_tag(index_name, filename=str(
                filename), fields=entry['key'])
            body = {
                "script": {
            ...
            res_update = es.update(
                index=index_name, id=str(filename), body=body)
