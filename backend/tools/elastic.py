...
es = Elasticsearch([{'host': 'elasticsearch', 'port': '9200','timeout':240, 'max_retries':10, 'retry_on_timeout':True}])
...
def get_tag(index_name: str, filename: str, fields: str) -> list:
    ...
    for token in tokenize:
        ...
        #text reconstruction list -> text
    text_clean=''
    for x in clean_words:
        text_clean= text_clean+ x +' '

    #TextRank
    keywTextRank=keywords.keywords(text_clean[100:],language="french",split=True,words=10)
            #avoid redundancies by filtering on stem
    keywTR=[]
    fr = SnowballStemmer('french')
    stem=[]
    for element in keywTextRank:
        if fr.stem(element) not in stem and len(element)>2:
            if element[-1]=='s':
                keywTR.append(element[:-1])
...
def get_tagOld(index_name: str, filename: str, fields: str) -> list:
