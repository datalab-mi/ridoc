...
filename="BF2014-14-14004 Etudiants étrangers.pdf"
...
clean_words = []
for token in return_token(textbrut):
...
keywRake=rake.apply(text_clean,text_for_stopwords=None)


#yake
yakefrench=yake.KeywordExtractor(lan="fr", windowsSize=1, top=5,n=3)
keywYake=yakefrench.extract_keywords(text_clean)
...
for element in keywTextRank:
    if fr.stem(element) not in stem:
