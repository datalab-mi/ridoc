	...
	<BaseItem {cssClass} {_id}>
		...
		<div slot="button">
			...
			{#if ($user.role === "admin") }
				...
				<button on:click={handleDelete} class="hover:bg-gray-400">
				...
				{#if (readonly && send) }
					<button on:click={handleSave} class="hover:bg-gray-400">
				...
				{:else if (!readonly && !send) }
					...
					<button on:click={handleCancel} class="hover:bg-gray-400">
				...
				{:else}
					<button on:click="{() => readonly = !readonly}" class="hover:bg-gray-400">
						<svg class="fill-current w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M12.3 3.7l4 4L4 20H0v-4L12.3 3.7zm1.4-1.4L16 0l4 4-2.3 2.3-4-4z"/></svg>
						<span>MODIFIER</span>
					</button>
				{/if}
