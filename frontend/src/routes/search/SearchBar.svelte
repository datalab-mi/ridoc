...
<div>
	<div class='text-3xl text-white left-0  m-6 '><b class="underline"> Rechercher</b> un rapport  <span class="uppercase">{$envJson.index_name}</span></div>
</div>
{#if $searchJson.length > 0}

	<!-- recherche basique, première ligne du tableau -->
	<div class="flex flex-row space-x-0 gap-0 justify-between bg-white w-4/12 m-4 ">
		{#each $searchJson[0] as { fields, value, type, placeholder, innerHtml, style, color, suggest }, j}
			{#if type == "button"}
				<div class="flex-none my-auto bg-white" >
					<button on:click={handleSearch} class="bg-white hover:bg-white text-gray-800">
						<svg class="fill-blue-600 w-6 h-6 sm:mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M12.9 14.32a8 8 0 1 1 1.41-1.41l5.35 5.33-1.42 1.42-5.33-5.34zM8 14A6 6 0 1 0 8 2a6 6 0 0 0 0 12z"/></svg>
					</button>
				</div>
			{:else if type === 'keyword' }
				<SearchKeywordInput bind:value={value} {fields} {placeholder} {color} />
			{:else if type === 'search' && suggest }
				<SearchSuggestInput bind:value={value} {placeholder} {innerHtml} {fields} style="w-2/4 m-4 " />
			{:else }
